...
public class LIBORMarketModelFromCovarianceModel extends AbstractProcessModel implements LIBORMarketModel, Serializable {
	...
	public RandomVariable[] getDrift(final int timeIndex, final RandomVariable[] realizationAtTimeIndex, final RandomVariable[] realizationPredictor) {
		...
		int		firstLiborIndex		= this.getLiborPeriodIndex(time)+1; // MERCURIO: senza il + 1
		if(firstLiborIndex<0) {
			firstLiborIndex = -firstLiborIndex-1 + 1;					// MERCURIO: anzichè il + 1 metti -1 (perchè hai anche rimosso il + 1 sopra)
		}
...
// ----> nota molto bene che i component index iniziano ad essere inizializzati solo da firstLiborIndex, mentre i  component da 0 a firstLiborIndex restano NULL!!
...
// IMPORTANT STEP		// If this is a model primitive then return it

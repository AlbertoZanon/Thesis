...
public class LIBORMarketModelStandard extends AbstractProcessModel implements LIBORMarketModel {
	...
	public RandomVariable getNumeraire(final double time) throws CalculationException {
		...
		if(timeIndex < 0) {
			// Interpolation of : linear interpolation of the reciprocal.
		...
		// The product 		
// calcola la produttoria!!		
		for(int liborIndex = firstLiborIndex; liborIndex<=lastLiborIndex; liborIndex++) {
// tiene in considerazion che, il valore del LIBOR resta costante una volta che t>T_i
	...
	//penso calcoli P(m(t)+1;t) come un forward bond = P(m(t)+1;0)/P(t;0)
